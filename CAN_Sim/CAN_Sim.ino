...
static void loop(int packetNum, int inputPackets[]) {
  int packetHexNum = 0x0CFFF048 + (packetNum * 256);
  int highByte[4];
  int lowByte[4];
  for (int i = 0; i < 4; i++){
    highByte[i] = inputPackets[i] / 256; //rpm highByte, tps highByte, etc...
    lowByte[i] = inputPackets[i] % 256; //rpm lowByte, tps lowByte, etc...
  }
